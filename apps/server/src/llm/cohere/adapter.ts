  ...
  async *streamResponse(
    ...
    for await (const event of stream) {
      ...
      if (event.type === "tool-call-delta") {
        ...
        if (acc) {
          const argDelta = e.delta?.message?.toolCalls?.[0]?.function?.arguments ?? ""
...
function toCohereTools(tools: Parameters<LLMProvider["streamResponse"]>[0]["tools"]) {
  return tools.map((t) => ({
...
function toCohereMessages(messages: UnifiedMessage[], system: string): Record<string, unknown>[] {
