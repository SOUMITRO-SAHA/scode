...
export class GeminiAdapter implements LLMProvider {
  ...
  async *streamResponse(
    ...
    let aggregatedFunctionCalls: Array<{ name: string; args: Record<string, unknown> }> = []
...
function toGeminiTools(tools: Parameters<LLMProvider["streamResponse"]>[0]["tools"]): FunctionDeclaration[] {
  return tools.map((t) => ({
    name: t.name,
...
function toGeminiContents(messages: UnifiedMessage[]): {
  ...
  for (const msg of messages) {
    if (msg.role === "system") {
      systemInstruction = typeof msg.content === "string" ? msg.content : msg.content.map((b) => ("text" in b ? b.text : "")).join("")
      continue
    }
    ...
    const text = typeof msg.content === "string" ? msg.content : msg.content.map((b) => ("text" in b ? b.text : "")).join("")
