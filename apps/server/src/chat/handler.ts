...
export async function handleChat(
  ...
  if (!session) {
    session = deps.sessionManager.create(prompt.slice(0, 60), resolvedModel, provider.id)
  }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    for await (const event of generator) {
      if (event.type === "text") {
        ...
        conversation.push({
          role: "assistant",
          content: [{ type: "tool_use", id: event.toolCall.id, name: event.toolCall.name, input: event.toolCall.input }],
        })
        conversation.push({ role: "tool", tool_call_id: event.toolCall.id, content: JSON.stringify(result) })
  ...
  deps.sessionManager.addMessage(session.id, { role: "assistant", content: fullResponse })
